...
formulario.addEventListener("submit", function (e) {
    ...
    if (nombre !== "" && correo !== "" && mensaje !== "") {
        // Todos los campos están llenos, se puede enviar el mensaje
        setTimeout()
...
// Mostrar u ocultar el botón del modal en función del tamaño de la pantalla
window.addEventListener("resize", function () {
  if (window.innerWidth <= 767) {
    ...
    navbarModal.style.display = "none"; // Ocultar el modal en pantallas grandes
  }
});

// Inicialmente, ocultar el botón en pantallas grandes
if (window.innerWidth > 767) {
  openModalButton.style.display = "none";
}
...
/* //formulario
const formuario= document.getElementById ('contacto-form')
const nomyapellInput= document.getElementById ('nombre')
const emailInput= document.getElementById ('correo')
const msjInput= document.getElementById ('mensaje')
const formGroup = document.querySelectorAll('.form-group')

//usuarios al localstorage
...
const saveToLocalStorage = () =>{
 ...
 //funcion para chequear si el campo esta vacio
  ...
  const showError = (input, message) => {
    ...
    form-group.classList.remove("success")
    form-group.classList.add("error")
    const error = form-group.querySelector("small")
    error.style.display ="block"
    error.textContent = message
  }
  //funcion para mostrar que u input es valido
   */